...
pub fn addr_in_bss(pe: &File, vaddr: u64) -> bool {
    match pe.section_by_name(".bss") {
...
pub fn vaddr_conv(pe: &File, vaddr: u64) -> Result<u64> {
...
fn process_relocations<'data>(pe: &File, data: &'data [u8]) -> Result<Vec<u8>> {
    let mut coff_rel = data.to_vec();

    if let Some(relocations) = pe.dynamic_relocations() {
        for (addr, rel) in relocations {
            ...
            cur.set_position(vaddr_conv(pe, addr)?);
...
impl<'pe, 'data> PeReader<'pe, 'data> {
    fn new(pe: &'pe File<'data>, pe_rel: &'data [u8]) -> Self {
        Self { pe, pe_rel }
    }

    fn make_cur(&self, vaddr: u64) -> Result<Cursor<&[u8]>> {
        let pos = vaddr_conv(self.pe, vaddr)?;
        let mut cur = Cursor::new(self.pe_rel);
        cur.set_position(pos);
        Ok(cur)
    }

    fn get_str(&self, vaddr: u64) -> Result<&'data str> {
        let ptr = vaddr_conv(self.pe, vaddr)?;
        get_str(self.pe_rel, ptr as usize)
    }
}

fn read_arr<T>(reader: &PeReader, vaddr: u64, len: usize) -> Result<Vec<T>>
where
    T: BinRead,
{
    if len == 0 {
...
fn read_len_arr<T>(reader: &PeReader, cur: &mut Cursor<&[u8]>) -> Result<Vec<T>>
where
    T: BinRead,
{
...
fn read_len_arr_nullable<T>(reader: &PeReader, cur: &mut Cursor<&[u8]>) -> Result<Vec<T>>
where
...
impl<'data> Il2CppCodeGenModule<'data> {
    fn read<'pe>(reader: &PeReader<'pe, 'data>, vaddr: u64) -> Result<Self> {
        let mut cur = reader.make_cur(vaddr)?;
...
impl<'data> Il2CppCodeRegistration<'data> {
    fn read(pe: &File<'data>, data: &'data [u8], addr: u64) -> Result<Self> {
        let reader = PeReader::new(pe, data);
...
impl Il2CppGenericInst {
    fn read(reader: &PeReader, vaddr: u64, types_map: &HashMap<u64, usize>) -> Result<Self> {
        let mut cur = reader.make_cur(vaddr)?;
...
impl Il2CppMetadataRegistration {
    fn read(pe: &File, data: &[u8], addr: u64, metadata: &GlobalMetadata) -> Result<Self> {
        let reader = PeReader::new(pe, data);
        let mut cur = reader.make_cur(addr)?;
...
impl<'data> RuntimeMetadata<'data> {
    ...
    pub fn read(pe: &File<'data>, data: &'data [u8], global_metadata: &GlobalMetadata) -> Result<Self> {
